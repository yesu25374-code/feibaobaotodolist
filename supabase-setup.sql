...
create table if not exists public.xiaoxiaoyu_sync_history (
...
alter table public.xiaoxiaoyu_sync_history enable row level security;
...
drop policy if exists "xiaoxiaoyu sync history deny read" on public.xiaoxiaoyu_sync_history;
drop function if exists public.xiaoxiaoyu_pull(text);
drop function if exists public.xiaoxiaoyu_push(text, jsonb, timestamptz);
drop function if exists public.xiaoxiaoyu_sync_backup();

create or replace function public.xiaoxiaoyu_sync_backup()
returns trigger
language plpgsql
security definer
...
begin
  if old.data is distinct from new.data then
    insert into public.xiaoxiaoyu_sync_history(sync_code, data, updated_at)
    values (old.sync_code, old.data, old.updated_at);
  end if;
  return new;
end;
$$;

drop trigger if exists xiaoxiaoyu_sync_backup_trigger on public.xiaoxiaoyu_sync;
create trigger xiaoxiaoyu_sync_backup_trigger
before update on public.xiaoxiaoyu_sync
for each row execute function public.xiaoxiaoyu_sync_backup();
...
create policy "xiaoxiaoyu sync history deny read"
on public.xiaoxiaoyu_sync_history
for select
to anon
using (false);
...
revoke all on public.xiaoxiaoyu_sync_history from anon, authenticated;
